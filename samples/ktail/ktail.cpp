...
header select { font: inherit; padding: .3em .5em; border: 1px solid var(--border); border-radius: 6px; background: var(--panel); color: var(--fg); }
...
(() => {
	...
	// the language choice goes to the application, which sets the cookie and comes back to this page
	const lang = document.getElementById('lang');
	if (lang) lang.addEventListener('change', () => lang.form.submit());
...
: m_Config(std::move(Config))
{
	...
	// the texts of the page and the menus, en and de. Check() names what a
	// translation lacks, once at startup
	ktail_strings::AddEmbeddedStrings(m_Strings);

	auto jFindings = m_Strings.Check();

	if (!jFindings.empty())
	{
		kDebug(1, "strings: {}", jFindings.dump());
	}
	...
	// starts with "@" is an identifier in the catalog
	Options.jMenus    = kjson::Parse(R"json([
		{ "title": "@menu.file", "items": [
			{ "title": "@menu.saveNote",     "key": "s", "action": "saveNote" },
			{ "title": "@menu.downloadFile", "key": "d", "action": "downloadFile" },
			{ "separator": true },
			{ "title": "@menu.revealFile",   "key": "r", "action": "revealFile" }
		] },
		{ "title": "@menu.view", "items": [
			{ "title": "@menu.toggleTheme",  "key": "t", "action": "toggleTheme" }
		] }
	])json");
...
//-----------------------------------------------------------------------------
{
	...
	// the texts in the language of the request: the cookie, Accept-Language, else the window's
	auto T = m_App->Text(HTTP);

	html::Page Page(m_Config.sTitle, T.GetTag());
	...
	// the language: the choice goes to KWebApp, which sets the cookie and returns to this page
	{
		auto Lang = Header.Add<html::Form>(KWebApp::LanguagePath, "lang");
		Lang.SetMethod(html::Form::POST);
		Lang.Add<html::Input>("next", PageLink(sDir, bHaveFile ? sFile : KString{}), html::Input::HIDDEN);

		auto Select = Lang.Add<html::Select>("lang", 1, html::Classes{}, "lang");
		Select.SetAttribute("title", T("tail.language"));

		for (const auto& sTag : m_Strings.GetLanguages())
		{
			Select.Add<html::Option>(m_Strings.Get(sTag, "lang.name"), sTag).SetSelected(sTag == T.GetTag());
		}
	}
	...
	SignOut.Add<html::Button>(T("tail.signOut"));
	...
	// left: the directory
	{
		...
		Head.Add<html::TableHeader>(T("tail.name"));
		Head.Add<html::TableHeader>(T("tail.size"));
		Head.Add<html::TableHeader>(T("tail.modified"));
	...
	// right: the tail of the selected file, and its note
	{
		...
		if (bHaveFile)
		{
			...
			Label.AddText(T("tail.notifyOnLines"));

			Bar.Add<html::Button>(T("tail.reveal"), html::Button::BUTTON, "native-only", "reveal");
			...
			Bar.Add<html::Link>(sDownload, T("tail.download"), "button", "download").SetDownload();
			...
			Form.Add<html::TextArea>("text", sNote, html::Classes{}, "note").SetAttribute("rows", "2").SetPlaceholder(T("tail.note"));
			Form.Add<html::Button>(T("tail.saveNote"));
		}
		else
		{
			Section.Add<html::Paragraph>("hint").AddText(T("tail.pickFile"));
		}
	}

	Page.Add<html::Footer>(html::Classes{}, "status").AddText(kFormat("{} {}", m_Config.sTitle, m_Config.sVersion));

	// the live connection to the application, the texts of the "tail" namespace as
	// window.kText, then the page script with its parameters as data attributes
	Page.Body().Add<html::Script>().SetAttribute("src", "/_kwa/live.js");
	m_App->AddStrings(Page, HTTP, "tail");
...
//-----------------------------------------------------------------------------
{
	...
	try
	{
		...
		Config.sLanguage     = Options("lang <tag>            : the language of the window's menus and notifications, default: the choice of the last run, else the system's", "");
