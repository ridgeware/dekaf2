...
| Tray       | `Options.bTrayIcon`, `jTrayMenu` (actions `show`, `quit` built in), `sTrayIcon` (file, macOS also `sf:<symbol>`), `SetTrayIcon(icon)` to swap the image                                 |                                                                                                                                                                                                                                                         |
| Background | `Options.bBackgroundActivity`: the page keeps running while the window is hidden, minimized or on another desktop (macOS: no web process suppression, no App Nap)                       |                                                                                                                                                                                                                                                         |
| Site paths | `Options.SitePaths`: paths the hosted site itself may call - with its origin (from `AllowedOrigins`) and the token as query parameter `token` - for what only the shell can produce     |                                                                                                                                                                                                                                                         |
| Dialog     | `Confirm()`: a yes-or-no question in the system's own dialog, where the user is (macOS: the active space, Windows: the current virtual desktop), for the page as `kNative.confirm({title, text, ok, cancel})` |                                                                                                                                                                                                                                   |
...
| Tray symbol with menu                                           | menu bar status item; SF Symbols as template images                                                             | not available (GTK 4 has no status icons; StatusNotifierItem over D-Bus would be the way)     | notification area icon (ICO files), menu on right click                                     |
| Attention                                                       | dock icon bounce                                                                                                | urgency hint on GTK 3; not available on GTK 4                                                 | taskbar flash                                                                               |
| Confirm dialog                                                  | NSAlert, moved to the active space                                                                              | GTK message dialog                                                                            | task dialog (common controls v6), else MessageBox                                           |
...
Status: macOS, Windows, and Linux with GTK 4 are tested. The GTK 3 path layer
compiles against its headers but has not been run yet.
