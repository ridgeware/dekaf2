...
TEST_CASE ("KURL")
{
    ...
    //||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
        ...
        SECTION ("Protocol table entries with default ports")
        {
            struct Expected
            {
                KStringView             sURL;
            ...
            CHECK ( sizeof(Table)/sizeof(Table[0]) == dekaf2::url::KProtocol::UNKNOWN - 1 );
...
TEST_CASE ("KURL2") // formerly missing
{
    ...
    SECTION("KURL new design")
    {
        ...
        CHECK ( sSerialized == svURL );
        CHECK ( ( URL.Protocol.getProtocol() == dekaf2::url::KProtocol::HTTPS ) ); // inner parens for clang 7
